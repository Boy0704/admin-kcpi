					...
					<div class="col-md-12">
						<div class="card">
							...
							<div class="card-body">
								...
								<br /><br />
								<vue-good-table :columns="columns" :rows="categories" :pagination-options="{
									enabled: true,
								}" :search-options="{
	enabled: true
}">
									<template slot="table-row" slot-scope="props">
										...
										<span v-if="props.column.field == 'desc'">{{ props.row.desc }}</span>
					...
					<form @submit.prevent="onSubmit">
						<div class="modal-body">
							...
							<div class="form-group">
								...
								<input type="text" class="form-control" placeholder="Masukkan Judul Kategori"
							...
							<div class="form-group">
								...
								<textarea v-model="formData.desc" class="form-control" cols="30" rows="10"></textarea>
							</div>
							<div class="form-group">
								<label>Type</label>
								<select class="form-control" v-model="formData.type">
									<option value="">Pilih type</option>
									<option v-for="item in type_category" :value="item.name">{{ item.name }}</option>
								</select>
...
export default {
	...
	methods: {
		...
		async onSubmit() {
			...
			if (this.existFile) {
				formReq.append('file_category', this.formData.image)
			}
			formReq.append('category', this.formData.category)
			formReq.append('desc', this.formData.desc)
			formReq.append('type', this.formData.type)
			formReq.append('publish', this.formData.publish ? 'y' : 't')
			if (this.isEdit == false) {
				await this.$axios
				.$post('/category', formReq, {
					headers: {
				...
				await this.$axios
				.$put('/category/'+this.idEdit, formReq, {
					headers: {
						'Content-Type': 'multipart/form-data'
					}
				})
